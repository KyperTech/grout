...
var Action = function () {
  ...
  _createClass(Action, [{
    ...
    value: function search(q) {
      ...
      return request.get(this.url + '/search/' + q).then(function (res) {
        if ((0, _lodash.has)(res, 'error')) {
          logger.error({
            description: 'Error in removal request.', action: _this3,
            res: res, func: 'remove', obj: 'Action'
          });
          return Promise.reject(res.error || res);
        }
        logger.log({
          description: 'Remove successful.', res: res, func: 'remove', obj: 'Action'
        });
        return res;
      }, function (error) {
        logger.error({
          description: 'Error in request for removal.', action: _this3,
          error: error, func: 'remove', obj: 'Action'
        });
        return Promise.reject(error);
      });
