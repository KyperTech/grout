...
export default class Action {
  ...
  init(actionData) {
    logger.log({
      description: 'Init action called.', actionData: actionData,
      func: 'url', obj: 'Action'
    });
    ...
    if (!this.isList) {
      ...
      if (isString(actionData)) { //String username provided
        ...
        logger.warn({
          description: 'Invalid action data object.', func: 'constructor', obj: 'Action'
        });
        this.isList = false;
  ...
  get() {
    return request.get(this.url).then((res) => {
      logger.log({
        description: 'Get responded successfully.', res: res,
        func: 'get', obj: 'Action'
      });
      if (has(res, 'error')) {
        logger.error({
      ...
      logger.error({
        description: 'Error in GET request.',
        error: error, func: 'get', obj: 'Action'
      });
  ...
  /** Update
   * @param {Object} newData - Object containing data to update with
   * @return {Promise}
   */
  update(updateData) {
